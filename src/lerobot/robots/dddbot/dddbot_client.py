...
from .config_dddbot import DddBotClientConfig


class DddBotClient(Robot):
    config_class = DddBotClientConfig
    name = "dddbot_client"

    def __init__(self, config: DddBotClientConfig):
        import zmq
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "Client is already connected. Do not run `robot.connect()` twice."
            )
        ...
        if self.zmq_observation_socket not in socks or socks[self.zmq_observation_socket] != zmq.POLLIN:
            raise DeviceNotConnectedError("Timeout waiting for Host to connect expired.")
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError("Client is not connected. You need to run `robot.connect()`.")
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command to move to a target joint configuration. Translates to motor space + sends over ZMQ"""
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "Client is not connected. You need to run `robot.connect()`."
            )
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "Client is not connected. You need to run `robot.connect()` before disconnecting."
            )
