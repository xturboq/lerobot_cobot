...
class LeKiwiClient(Robot):
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "Cobot Daemon is already connected. Do not run `robot.connect()` twice."
            )
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError("CobotClient is not connected. You need to run `robot.connect()`.")
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command Cobot to move to a target joint configuration. Translates to motor space + sends over ZMQ
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "Cobot is not connected. You need to run `robot.connect()` before disconnecting."
            )
